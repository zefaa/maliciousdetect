...
import static routing.MessageRouter.DENIED_NO_SPACE;
import static routing.MessageRouter.DENIED_OLD;
import static routing.MessageRouter.DENIED_TTL;
 ...
 * {@link #update()}).
 */
public abstract class ActiveRouter1 extends MessageRouter {
     ...
     */
    public ActiveRouter1(Settings s) {
        super(s);
     ...
     */
    protected ActiveRouter1(ActiveRouter1 r) {
        super(r);
    ...
    @Override
    public Message messageTransferred(String id, DTNHost from) {// mengambil dari message transfered pada message router 
     ...
     */
    protected int startTransfer(Message m, Connection con) { // memulai transfer yaitu pesan dari mana diconnection kan ke mana 
        int retVal;
        ...
        if (String.valueOf(con.getOtherNode(getHost()).toString().charAt(0)).equals("s")) {
            retVal = DENIED_DELIVERED;

        } else {
//            System.out.println(con.getOtherNode(getHost()).toString());
        ...
        // System.out.println("Coneksinya = \t "+con+"\tPesan = \t"+m+"\t"+retVal);
     ...
     */
    protected boolean canStartTransfer() {  // membuat pemeriksaan dasar (bahwa kami memiliki setidaknya satu pesan dan satu koneksi) tentang bisakah router ini memulai transfer
     ...
     * {@link MessageRouter#RCV_OK} if receiving seems to be OK, TRY_LATER_BUSY
     * if router is transferring, DENIED_OLD if the router is already carrying
     * the message or it has been delivered to this router (as final recipient),
     * or DENIED_NO_SPACE if the message does not fit into buffer
     */
    protected int checkReceiving(Message m) { // menlakukan pengecekan pada pesan ketika diterima 
        if (isTransferring()) {
     ...
     */
    protected boolean makeRoomForMessage(int size) { //Menghapus pesan dari buffer (yang terlama terlebih dahulu) hingga ada cukup ruang untuk pesan baru.
        ...
        while (freeBuffer > size) {
            Message m = getOldestMessage(false); // don't remove msgs being sent
     ...
     */
    protected void makeRoomForNewMessage(int size) {// membuat ruang untuk pesan baru 
        
            makeRoomForMessage(size);// ukuran tempat untuk pesan 
     ...
     * sent from the oldest message check (i.e. if oldest message is being sent,
     * the second oldest message is returned) Jika benar, tidak termasuk pesan
     * yang dikirim dari pemeriksaan pesan terlama (misalnya jika pesan terlama
     * dikirim, pesan terlama kedua dikembalikan
     ...
     */
    protected Message getOldestMessage(boolean excludeMsgBeingSent) {// Mengembalikan pesan terlama (dengan waktu terima) di buffer pesan (yang tidak dikirim jika mengecualikan Pesan yang Dikirim adalah benar)
 ...
 /*List<Connection> connections = getConnections();
     ...
     * (checking every one simulated minute). @see
     * #addToSendingConnections(Connecti
     *
     * on)
     */
    @Override
    public void update() {
        ...
        for (int i = 0; i < this.sendingConnections.size();) {
            ...
            if (con.isMessageTransferred()) {
                ...
                removeCurrent = true;
            } /* remove connections that have gone down */ else if (!con.isUp()) {
                if (con.getMessage() != null) {
                    transferAborted(con);
                    con.abortTransfer();
                }
     ...
     * {@link #update()}. Subclasses that are interested of the event may want
     * to override this.
     *
