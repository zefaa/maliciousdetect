...
/**
 * <p>
 * Computes the global and local centrality of a node using the CWindow
 * centrality algorithm described in <em>BUBBLE Rap: Social-based Forwarding in
 * Delay Tolerant Networks</em> by Pan Hui et al. (2008) (the bibtex record is
 * included below for convenience).</p>
 *
 * <p>
 * CWindow estimates the node's centrality using the node's average degree over
 * a set of time windows, i.e. it counts the unique encounters of the node over
 * the most recent time interval, say the last 6 hours, then averages it with
 * the counts from the six hour interval prior to the most recent one, the one
 * prior to that, and so on for some number of intervals. The authors found that
 * a six hour time window correlated well with a node's actual centrality value
 * (for the datasets they examined), which is the default interval, but the
 * setting is configurable along with the number of intervals to consider.
 * </p>
 *
 * <p>
 * This computation is done at regular intervals instead of every time the
 * global and local centrality measures are requested.</p>
 *
 * <p>
 * This class looks for three settings:
 * <ul>
 ...
 * between upd
 * ates to the centrality values. A longer interval reduces simulation time at
 * the expense of accuracy. Default: 10 minutes</li>
 * </ul>
 * </p>
 * <pre>
 ...
 * @see Centrality
 */
public class CWindowCentrality_UAS implements CentralityKu {
    ...
    protected int[] arrayGlobalCentrality;

    public CWindowCentrality_UAS(Settings s) {
    ...
    public CWindowCentrality_UAS(CWindowCentrality_UAS proto) {
...
//		System.out.println(this+".localCentrality: "+this.localCentrality+"\n");
    ...
    public CentralityKu replicate() {
        return new CWindowCentrality_UAS(this);
    }
