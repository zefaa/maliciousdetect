 ...
 * be configured.
 */
public class MessageEventGeneratorMal implements EventQueue {
	...
	public static final String MESSAGE_TIME_S = "time";
         ...
         * 
         */
        public static final String JPESAN = "rankMessage";
	 ...
	 */
	public MessageEventGeneratorMal(Settings s){
		...
		this.idPrefix = s.getSetting(MESSAGE_ID_PREFIX_S);
                this.jpesan = s.getCsvInts(JPESAN); //
		...
		if (this.sizeRange.length == 1) {
			...
			s.assertValidRange(this.sizeRange, MESSAGE_SIZE_S);
                ...
                if (this.jpesan.length == 1) {
			/* convert single value to range with 0 length */
			this.jpesan = new int[] {this.jpesan[0], this.jpesan[0]};
		}
		else {
			s.assertValidRange(this.jpesan, JPESAN);
		}
	...
	protected int drawMessageSize() {
        ...
        /**
         * mengacak jumlah pangkat pesan yang dibuat oleh node. 
         * @return 
         */
        protected int drawJumPesan() {
            //operasi ternary. ekspresi ? nilai jika true : nilai jika false
		int jumDiff = jpesan[0] == jpesan[1] ? 0 :  // jika jumDiff sama dengan jpesan[0] atau jpesan[1] (kiri kanan sama). klo sama 0 kalo beda kanan-kiri
			rng.nextInt(jpesan[1] - jpesan[0]); //sehingga hasil pangkatnya jumlahnya sesuai kondisi
		return jpesan[0] + jumDiff;
	}
	...
	public ExternalEvent nextEvent() {
		...
		int hasilDrawPesan = drawJumPesan(); // hasilDrawPesan sama dengan method drawJumPesan. hasil dari method darwJumPesan disimpan dalam hasilDrawPesan. 
                jumPesan = (int) Math.pow(2, hasilDrawPesan); // jumPesan berisi hasil perpangkatan jumlah pesan yang dibuat. Pesan yang dibuat ada 2^hasilDrawPesan.
               //cetak jumlah pesan
               //System.out.println(jumPesan);
		interval = drawNextEventTimeDiff();
		
		/* Create event and advance to next event >> buat pesan */
		MessageCreateEventMal mce = new MessageCreateEventMal(from, to, this.getID(), 
				 jumPesan, responseSize, this.nextEventsTime, this.id, idPrefix, sizeRange[0], sizeRange[1]);
